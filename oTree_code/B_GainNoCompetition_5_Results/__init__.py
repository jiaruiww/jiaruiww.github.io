...
class C(BaseConstants):
    NAME_IN_URL = 'A2_GC_5_Results'
    PLAYERS_PER_GROUP = None
    NUM_ROUNDS = 1

    PROLIFIC_COMPLETION_CODE = 'CN1X3981'

    QUIZ_THRESHOLD_SCORE = 6

    # Fix: Update to correct page counts
    APP3_PAGES = 6  # A2_GC_3_Survey pages
    APP4_PAGES = 4  # A2_GC_4_Decision_P pages
    APP5_PAGES = 7  # Current app pages (fixed)
    TOTAL_PROGRESS_UNITS = APP3_PAGES + APP4_PAGES + APP5_PAGES  # = 21


# Add helper function to ensure prolific_id can always be retrieved
def ensure_prolific_id(player):
    """Ensure player's prolific_id field has a value, trying multiple sources"""
    ...
    if player.participant.label and player.participant.label.strip():
        ...
        # Also save to participant.vars for future use
...
class Subsession(BaseSubsession):
    def creating_session(self):
        ...
        for p in self.get_players():
            ...
            # Enhanced prolific_id retrieval using helper function
            p.prolific_id = ensure_prolific_id(p)

            # Fix: If APP4 completed, continue progress
            if p.participant.vars.get('app4_completed', False):
                p.participant.vars['current_progress'] = C.APP3_PAGES + C.APP4_PAGES  # = 10
...
class Player(BasePlayer):
    ...
    attention_check_response = models.IntegerField(
        ...
        blank=True  # Allow empty so users can leave unselected
    )
...
class AttentionCheckPage(Page):
    ...
    def vars_for_template(player):
        ...
        # Fix: Ensure progress continues from where app4 ended
        if (player.participant.vars.get('app4_completed', False) and
            ...
            # Force the progress to start where app4 ended
            player.participant.vars['current_progress'] = C.APP3_PAGES + C.APP4_PAGES  # = 10
    ...
    def before_next_page(player, timeout_happened):
        ...
        player.attention_check_time = time.time() - start_time
...
class CompletionPage(Page):
    ...
    def vars_for_template(player):
        ...
        # Get Prolific completion URL (usually set in settings.py SESSION_CONFIGS)
