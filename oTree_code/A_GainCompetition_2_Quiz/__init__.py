...
class C(BaseConstants):
    ...
    RETURN_CODE = 'C1FIVYUV'  # Code for matching timeout returns

    # Question images remapped according to the latest Excel file order
    QUESTION_IMAGES = {
    ...
    # Choice images remapped according to the latest Excel file order
    CHOICE_IMAGES = {
    ...
    # Correct answers remapped according to the latest Excel file order
    CORRECT_ANSWERS = {
        1: 2,  # B - Q2's answer
        2: 3,  # C - Q3's answer
        3: 1,  # A - Q20's answer
...
class Group(BaseGroup):
    ...
    def handle_tie(self):
        """Handle tie situation, ensure one player is winner and one is loser"""
        players = self.get_players()
        if len(players) != 2:
            return  # If there are not two players, don't execute

        p1, p2 = players

        # Randomly select one player as winner
...
class Player(BasePlayer):
    ...
    # Add this line in Player class, after wrong_answers_count field
    skipped_questions_count = models.IntegerField(initial=0)

    # Matching-related fields
    paired_partner_id = models.StringField(blank=True)  # Paired partner's participant.id
    paired_partner_prolific_id = models.StringField(blank=True)  # Paired partner's prolific_id
    ...
    opponent_score = models.FloatField(blank=True, null=True)  # Opponent score
    tie_occurred = models.BooleanField(initial=False)  # Whether tie occurred
    ...
    # Add this new field in Player class, after average_time_per_question field
...
class MatchingWaitPage(WaitPage):
    ...
    def vars_for_template(player: Player):
        ...
        if timeout_happened:
            ...
            # Persist timeout state to Player model
    ...
    def after_all_players_arrive(group: Group):
        ...
        if len(players) == 2:
            ...
            # Record prolific_id cross-reference
        ...
        for p in players:
            ...
            # Get prolific_id from participant.vars (three-layer protection mechanism)
            if 'prolific_id' in p.participant.vars:
                # Layer 1: Get previously stored ID from participant.vars
                p.prolific_id = p.participant.vars['prolific_id']
            elif p.participant.label:
                # Layer 2: Use participant.label (usually auto-filled by URL parameter)
                p.prolific_id = p.participant.label
            else:
                # Layer 3: Generate a unique identifier based on session and player ID
                ...
                # Record this exception for later analysis
...
class QuizPage(Page):
    ...
    def live_method(player: Player, data):
        """
        Enhanced live_method for production robustness
        """
        try:
            ...
            try:
                ...
                if start_time is None:
                    # If no start time, use current time and record
            ...
            # 5. Check time expiration logic (keep original logic)
            time_expired = False
            ...
            # 7. Build response data (ensure all fields have valid values, keep original field names)
            response_data = {
                'player_score': player_score,
                'teammate_score': opponent_score,  # Keep original field name
                'teammate_correct_count': opponent_correct_count,  # Keep original field name
                'remaining_time': remaining_time
            }

            # 8. If time expired, add time expiration signal (keep original logic)
            if time_expired:
            ...
            # Return minimal usable dataset (keep original field names and default values)
            return {
    ...
    def vars_for_template(player: Player):
        ...
        # Fix: Ensure all values are not None, provide safe defaults
        ...
        # NEW: Calculate progress bar width percentage (ensure always returns valid integer)
        ...
        return {
            ...
            'teammate_bar_width': teammate_bar_width,  # Ensure is valid integer
            'remaining_time': remaining_time,
        ...
        if player.round_number == 1 and 'quiz_start_time' not in player.participant.vars:
             ...
             if player.group:
                # If quiz_start_timestamp is not yet set, set it
                 ...
                 # Use group's synchronized timestamp as each player's quiz start time
    ...
    def before_next_page(player: Player, timeout_happened):
        # Add exception exit data save mechanism
        try:
            current_time = time.time()

            # ==========================================================================
            # NEW: Update variables every round - new feature (consistent with A2/A4)
            # ==========================================================================

            # 1. Time-related variables - update every round
            ...
            # real_quiz_duration = current actual time used (net answer time excluding interruptions)
            ...
            # 2. Detection status variables - update every round, provide current best estimate
            if hasattr(player, 'session_interruption_detected') and player.session_interruption_detected:
            ...
            if player.round_number > 1:
                ...
                if prev_timestamp_key in player.participant.vars:
                    ...
                    if time_gap > 30:  # Over 30 seconds considered possible interruption
            ...
            # 6. State snapshot (for exception recovery)
            ...
            # ==========================================================================
            # Original logic kept unchanged, but add per-round update functionality
            # ==========================================================================
            ...
            # Calculate time and save basic info
            ...
            # NEW: Initialize counters (ensure each round has correct initial values)
            ...
            # NEW: Update statistics in Player model every round (only update current round)
            ...
            # NEW: Update total_questions_attempted every round (only update current round)
            player.total_questions_attempted = player.participant.vars.get('questions_attempted', 0)

            # NEW: Update final_score every round (only update current round)
            player.final_score = player.participant.vars.get('score', C.INITIAL_SCORE)

            # Improved average time calculation logic (only update current round)
            questions_attempted = player.participant.vars.get('questions_attempted', 0)
            if questions_attempted > 0:  # Avoid division by zero
                ...
                # Remove cross-round sync: no longer update all rounds
            else:
            ...
            # NEW: Calculate average time including skipped questions (only update current round)
            ...
            # NEW: Update competition status-related variables every round (compare with opponent, only update current round)
            player_score = player.participant.vars.get('score', C.INITIAL_SCORE)
            opponent_score = C.INITIAL_SCORE
            ...
            if player_score > opponent_score:
                ...
                player.is_winner = False  # Temporarily set to False in tie, final round will handle
                player.competition_result = "Currently tied!"

            # Remove cross-round sync: no longer synchronously update all rounds' key variables
            ...
            # If it's the last round or time is up, ensure final results are recorded
            if player.round_number == C.NUM_ROUNDS or time_expired:
                ...
                if 'competition_results' not in player.participant.vars:
                    ...
                    if player_score > opponent_score:
                        ...
                        # Because handle_tie has already set is_winner, tie_occurred and other fields and competition_results
                        # So no need to set these fields again here
                        return

                    # Persist competition result info to Player model
            ...
            # FIX: Ensure existing data can be saved even if error occurs (per-round update)
            ...
            # NEW: Even if error occurs, update total_time_spent
            ...
            # NEW: Even if error occurs, update statistics (only update current round)
