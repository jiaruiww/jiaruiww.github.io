...
class C(BaseConstants):
    NAME_IN_URL = 'A1_GT_5_Results'
    PLAYERS_PER_GROUP = None
    NUM_ROUNDS = 1

    PROLIFIC_COMPLETION_CODE = 'C1BYR0Q0'

    APP3_PAGES = 6
    APP4_PAGES = 4
    APP5_PAGES = 7
    TOTAL_PROGRESS_UNITS = APP3_PAGES + APP4_PAGES + APP5_PAGES
    ...
    def creating_session(self):
        for p in self.get_players():
            ...
            if p.participant.vars.get('app4_completed', False):
                p.participant.vars['current_progress'] = C.APP3_PAGES + C.APP4_PAGES
...
class Player(BasePlayer):
    ...
    attention_check_time = models.FloatField(initial=0)
...
class AttentionCheckPage(Page):
    ...
    def vars_for_template(player):
        ...
        if player.participant.vars.get('app4_completed', False) and 'AttentionCheckPage_App5' not in player.participant.vars.get('pages_viewed', set()):
            player.participant.vars['current_progress'] = C.APP3_PAGES + C.APP4_PAGES
