...
class C(BaseConstants):
    NAME_IN_URL = 'A2_GC_4_Decision_P'
    PLAYERS_PER_GROUP = 2
    NUM_ROUNDS = 1
    ...
    APP3_PAGES = 6
    APP4_PAGES = 4
    TOTAL_PROGRESS_UNITS = APP3_PAGES + APP4_PAGES
...
class Player(BasePlayer):
    ...
    decision_time = models.FloatField(initial=0)
    belief_time = models.FloatField(initial=0)
    dictator_game_time = models.FloatField(initial=0)
    ...
    # Keep original slider value for form processing (maintains backward compatibility)
...
class DictatorGame(Page):
    ...
    def before_next_page(player, timeout_happened):
        ...
        # allocation_amount represents amount given to other person (slider value)
        ...
        # Also maintain storage in participant.vars (for cross-app data passing, maintains backward compatibility)
        player.participant.vars['dictator_other_amount'] = other_amount
        player.participant.vars['dictator_your_amount'] = your_amount
        player.participant.vars['dictator_self_amount'] = your_amount
