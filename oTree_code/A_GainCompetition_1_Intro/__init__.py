...
class C(BaseConstants):
    ...
    INTRO_COMP_CHOICES = [
        ("A", "Lucy"),
        ("B", "Emily"),
        ("C", "It's a tie. One of them will be randomly selected to receive the reward."),
    ]
...
class Player(BasePlayer):
    ...
    time_welcome_page = models.IntegerField(blank=True, null=True)
    time_first_page = models.IntegerField(blank=True, null=True)
    time_prolific_id_page = models.IntegerField(blank=True, null=True)

    # Cumulative time tracking from App 1
    time_until_first = models.IntegerField(blank=True, null=True)

    # Intro comprehension question
    intro_comp_answer = models.StringField(
        label="In the 4-minute IQ Quiz competition, "
              "Emily answers 20 questions (14 correct, 6 incorrect) while "
              "Lucy answers 12 questions (8 correct, 4 incorrect). Who <strong>receives the $4</strong> reward?",
    ...
    time_introduction = models.IntegerField(initial=0)  # Time spent on Introduction page (seconds)
    time_practice = models.IntegerField(initial=0)  # Time spent on Practice page (seconds)
    time_quiz_start = models.IntegerField(initial=0)  # Time spent on QuizAboutStart page (seconds)
    time_total = models.IntegerField(initial=0)  # Total time across all pages (seconds)
...
class Introduction(Page):
    ...
    @staticmethod
    def before_next_page(player, timeout_happened):
        # For correct answers, also increment counter and record
        if player.intro_comp_answer == "B":
            player.intro_comp_attempts += 1  # Always increment
...
class PracticeQuestion(Page):
    ...
    @staticmethod
    def before_next_page(player, timeout_happened):
        # Ensure correct answer is also recorded, and counter always increments
...
class QuizAboutStart(Page):
    ...
    def before_next_page(player, timeout_happened):
        ...
        player.app1_completed = True  # This field needs to be added to the Player class
